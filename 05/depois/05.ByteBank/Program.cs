...
namespace _05.ByteBank
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            //ex é a variável que vai capturar a exceção
            catch (Exception ex)
            {
                Console.WriteLine("Aconteceu um problema na transferência.");
                //gerando uma mensagem da ex
    ...
    class TransferenciaBancaria : ITransferenciaBancaria
    {
        public void Efetuar(ContaCorrente contaDebito, ContaCorrente contaCredito
            , decimal valor)
        {
            //lançando exceção com condições personalizada
            if (contaDebito == null)
            {
                //lançando uma exceção quando a contaDebito for nulo
            ...
            if (valor <= 0)
            {
                //lançando uma excessão fora da faixa
                //o argumento especificado está valor do intervalo dos valores validos
                throw new ArgumentOutOfRangeException("valor");
            }
            if (valor > contaDebito.Saldo)
            {
                //lançando um erro com um exceção criada/customizada
    ...
    class TransferenciaBancaria_BD : ITransferenciaBancaria
    {
            ...
            , decimal valor)
        {
            ...
            catch (SqlException ex) //EXCEÇÕES ESPECÍFICAS PRIMEIRO, CAPTURANDO UMA EXCEÇÃO DO TIPO SQL
            {
    ...
    public class SaldoInsuficienteException : Exception
    {
        ...
        //substituindo a mensagem de erro que é exibida quando a exceção é chamada/lançada
